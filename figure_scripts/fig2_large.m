...
sim_data_lin = importdata('sim_data/sim_data_lin.txt');
sim_data_tri = importdata('sim_data/sim_data_tri.txt');
...
set(gcf, 'Position', [pos(1) pos(2) 15 20],'PaperPositionMode','Auto',...
    'PaperUnits','Centimeters','PaperSize',[15 18],...
    'PaperPosition',[0 -1 15 20]) %[left bottom width height]
...
set(xlabp,'Position',get(xlabp,'Position')+[3.2e-4,0.2,0])
ylabel('Uptake')
xlabel('\omega\times10^{-4}')
ax = gca;
...
% Plot range
xlow=0; xhigh=100; ylow=0.65; yhigh=0.85;
...
% Import data
ac_struct = importdata('ac_data/ac_tri.txt');
ac = ac_struct.data;
...
% Plot range
xlow=0; xhigh=100; ylow=0.65; yhigh=0.9;
axis([xlow xhigh ylow yhigh ])
...
%-------------------
 ...
 %Export at 300 dpi 
 print('../../Research/vaccine_behaviour_16/demetri_figures/fig2_large','-dpdf','-r300')
