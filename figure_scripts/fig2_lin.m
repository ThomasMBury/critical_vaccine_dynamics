...
sim_data_lin = importdata('sim_data/simdata_lin.txt');
...
% Import data
sdD_struct = importdata('sd_data/sd_lin.txt');
sdD = sdD_struct.data;
var=[sdD(:,1),sdD(:,2:end).^2];

% Find mean and standard deviation
num_comps = size(var,2);
var_mean_lin = mean(var(:,2:num_comps),2);
var_deviation_lin = std(var(:,2:num_comps),0,2);

time_indices = var(:,1);
% Find Kendall Tau of mean ac (2dp)
[corMatrix,pvals] = corr([time_indices,var_mean_lin],'type','Kendall');
ktau_lin = round(corMatrix(1,2),2);
...
% Import data
cov_struct = importdata('cov_data/cov_lin.txt');
...
%-------------------
 ...
 %Export at 300 dpi 
 print('../../Research/vaccine_behaviour_16/PNAS_demetri/figures/fig2_lin','-dpdf','-r300')
