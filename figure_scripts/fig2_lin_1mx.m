% Script to plot column of time series including COV with and without
% detrending
...
sim_data_lin = importdata('sim_data/simdata_lin2.txt');
...
% Import data
ac_struct = importdata('ac_data/ac_lin_1mx.txt');
...
% Plot range
xlow=0; xhigh=100; ylow=0.6; yhigh=0.85;
...
% change ticks
ax = gca;
ax.YTick = [0.4 0.5 0.6,0.7,0.8 0.9];
...
% Import data
sdD_struct = importdata('sd_data/sd_lin_1mx.txt');
sdD = sdD_struct.data;

% Find mean and standard deviation
num_comps = size(sdD,2);
sd_mean_lin = mean(sdD(:,2:num_comps),2);
sd_deviation_lin = std(sdD(:,2:num_comps),0,2);

time_indices = sdD(:,1);
% Find Kendall Tau of mean ac (2dp)
[corMatrix,pvals] = corr([time_indices,sd_mean_lin],'type','Kendall');
ktau_lin = round(corMatrix(1,2),2);



% Plot with shaded error bars
shadedErrorBar(time_indices,sd_mean_lin,sd_deviation_lin,'k'); 
hold on
...
% Import data
cov_struct = importdata('cov_data/cov_lin_ND_1mx.txt');
cov_temp = cov_struct.data;

% Work with data up to tcrit
    tcrit = 80;
    time_indices = cov_temp(:,1);
    temp = abs(time_indices-tcrit);
    [val,idx] = min(temp);
    cov = cov_temp(1:idx,:);
...
cov_deviation_lin = std(cov(:,2:num_comps),0,2);

% Stop plot at t=tcrit i.e where threshold is passed
time_indices = cov(:,1);

% Find Kendall Tau of mean ac (2dp)
[corMatrix,pvals] = corr([time_indices,cov_mean_lin],'type','Kendall');
ktau_lin = round(corMatrix(1,2),2);
...
% change ticks
% ax = gca;
% ax.YTick = [0.5e-2 1e-2,1.5e-2,2e-2];
% ax.YTickLabel = {'0.5','1','1.5','2'};


% % base
% text(-0.1,2.15e-2,'\times10^{-2}','Color','k','FontSize',fs)
% 
% Draw window arrow
xpos_arrow = [0.135,0.28];
...
%-------------------
 ...
 %Export at 300 dpi 
 print('../../Research/vaccine_behaviour_16/PNAS_demetri/figures/fig2_lin_1mx','-dpdf','-r300')
