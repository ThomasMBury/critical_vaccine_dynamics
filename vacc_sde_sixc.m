function [ data_out ] = vacc_sde_sixc( y0, tmax, k, choose_risk, wl, wh, d, num_realisations, sigma )
...
%   Input arguments: 
%       y0 - intitial conition for y = [S,I,x,C], C is cumulative
%       infections
...
data_out=zeros([length(t),4*num_realisations+2]);
    % Time column
    data_out(:,1)=t;
...
for realisation_count = 1:num_realisations
    ...
    % Implement sdetools
    ops = sdeset('SDEType','Ito','RandSeed',seed, 'ConstGFUN','yes','NonNegative','yes','UBoundOne',[1,2,3]);
    y = sde_euler(system,G,t,y0,ops);

    % Save current data to the output array
    data_out(:,4*realisation_count-1 : 4*realisation_count+2)=y;
